...
fftwf_complex *field1, *field1_k, *field2, *field2_k;
fftwf_plan p1, p2;
...
static complex<double> epsilon1_k[N][N], epsilon2_k[N][N];
...
complex<double> sigma_plus(int i, int j, int i2, int j2, double r) {
    ...
    if (i==N | j==N | i2==N | j2==N) window=0; //in main, loops start at i=0, but there is no symmetrical term (sigma(N-i)) as it would have index N-i = N which does not exist in field array: need to remove this contribution from final sum by saying it is zero
        ...
        // bispectrum
        B=epsilon1_k[i2][j2]*epsilon1_k[i][j]*conj(epsilon2_k[sx][sy]);
        // p vector
...
int main() {
    ...
    string file1=string(filename_box1)+string(".txt");
    string file2=string(filename_box2)+string(".txt");

    field1 = (fftwf_complex*) fftwf_malloc(sizeof(fftwf_complex) * (N) * (N));
    field1_k = (fftwf_complex*) fftwf_malloc(sizeof(fftwf_complex) * N * (N));

    field2 = (fftwf_complex*) fftwf_malloc(sizeof(fftwf_complex) * (N) * (N));
    field2_k = (fftwf_complex*) fftwf_malloc(sizeof(fftwf_complex) * N * (N));

    //Reading field files
    ifstream field_file ;
    field_file.open(file1);
    if (!field_file.is_open() ) {cerr << "Unable to open file 1" <<endl; return -1;}
    cout << "Field file 1 opened: reading..." <<endl ;
    for (int i=0; i<N; i++) {
        for (int j=0; j<N; j++) {
            field_file >> field1[i + N * j][0];
            field1[i + N * j][1]=0;
            // cout << field[i + N * j][0] <<flush;
        }
    }
    cout << "Done reading file 1." <<endl;
    field_file.close();

    ifstream field_file2 ;
    field_file2.open(file2);
    if (!field_file2.is_open() ) {cerr << "Unable to open file 2" <<endl; return -1;}
    cout << "Field file 2 opened: reading..." <<endl ;
    for (int i=0; i<N; i++) {
        for (int j=0; j<N; j++) {
            field_file2 >> field2[i + N * j][0];
            field2[i + N * j][1]=0;
            // cout << field[i + N * j][0] <<flush;
        }
    }
    cout << "Done reading file 2, computing FFT..." <<endl;
    field_file2.close();

    p1 = fftwf_plan_dft_2d(N, N, field1, field1_k, FFTW_FORWARD, FFTW_ESTIMATE);
    fftwf_execute(p1);

    p2 = fftwf_plan_dft_2d(N, N, field2, field2_k, FFTW_FORWARD, FFTW_ESTIMATE);
    fftwf_execute(p2);

    // check
    cout << field1_k[12 + N * 12][0] << " " << field1_k[12 + N * 12][1] <<endl;
    cout << field1_k[(N-12) + N * (N-12)][0] << " " << field1_k[(N-12) + N * (N-12)][1] <<endl;
    double abs=0;
    for (int i=0; i<N; i++) {
        for (int j=0; j<N; j++) {
            abs = sqrt( pow(field1_k[i + N * j][0],2) + pow(field1_k[i + N * j][1],2) );
            if ( abs<delta_k) epsilon1_k[i][j]=0;
            else epsilon1_k[i][j] = complex<double>(field1_k[i + N * j][0],field1_k[i + N * j][1])/abs;
            abs = sqrt( pow(field2_k[i + N * j][0],2) + pow(field2_k[i + N * j][1],2) );
            if ( abs<delta_k) epsilon2_k[i][j]=0;
            else epsilon2_k[i][j] = complex<double>(field2_k[i + N * j][0],field2_k[i + N * j][1])/abs;
        }
    }

    string outfilename = string(filename_box1)+string("_L")+to_string(int(L))+string("_cross_tcf.txt");
    ofstream outfile;
